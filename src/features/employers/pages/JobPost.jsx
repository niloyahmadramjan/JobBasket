...
const JobPost = () => {
  ...
  const handleJobPost = (e) => {
    ...
    const {requirements,responsibilities,max,min,currency, ...jobData} =job;

    // formet data structure 
         jobData.salaryRange = {max,min,currency }
         const requirementsClearData = requirements.split(',').map(t=> t.trim());
         const responsibilitiesClearData = responsibilities.split(',').map(t=> t.trim());
    ...
    <section className="min-h-screen px-4 py-10 bg-base-100">
      <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-xl p-6 md:p-10">
        ...
        <form onSubmit={handleJobPost} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Job Type */}
          <div className="form-control w-full">
            <label className="label"><span className="label-text font-medium">Job Type</span></label>
          ...
          {/* Category */}
          <div className="form-control w-full">
            <label className="label"><span className="label-text font-medium">Job Category</span></label>
            <input type="text" name="category" className="input input-bordered w-full" required />
          </div>

          {/* Deadline */}
          <div className="form-control w-full">
            <label className="label"><span className="label-text font-medium">Application Deadline</span></label>
            <input type="date" name="applicationDeadline" className="input input-bordered w-full" required />
          </div>

          {/* Salary Min */}
          <div className="form-control w-full">
            <label className="label"><span className="label-text font-medium">Min Salary</span></label>
            <input type="number" name="min" className="input input-bordered w-full" required />
          </div>

          {/* Salary Max */}
          <div className="form-control w-full">
            <label className="label"><span className="label-text font-medium">Max Salary</span></label>
          ...
          {/* HR Name */}
          <div className="form-control w-full">
            <label className="label"><span className="label-text font-medium">HR Name</span></label>
            <input type="text" name="hr_name" defaultValue={user.displayName} className="input input-bordered w-full" required />
          </div>

          {/* HR Email */}
          <div className="form-control w-full">
            <label className="label"><span className="label-text font-medium">HR Email</span></label>
            <input type="email" name="hr_email" defaultValue={user.email} className="input input-bordered w-full" required />
          </div>

          {/* Company Logo */}
          <div className="form-control w-full">
            <label className="label"><span className="label-text font-medium">Company Logo URL</span></label>
            <input type="url" name="company_logo" className="input input-bordered w-full" />
          </div>
          ...
          {/* Requirements */}
          <div className="form-control md:col-span-2">
            <label className="label"><span className="label-text font-medium">Requirements (comma separated)</span></label>
            <textarea name="requirements" className="textarea textarea-bordered w-full" rows="3" required />
          </div>

          {/* Responsibilities */}
          <div className="form-control md:col-span-2">
            <label className="label"><span className="label-text font-medium">Responsibilities (comma separated)</span></label>
            <textarea name="responsibilities" className="textarea textarea-bordered w-full" rows="3" required />
          </div>
